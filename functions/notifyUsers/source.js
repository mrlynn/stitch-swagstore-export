exports = function(event){
  ...
  let inStockNotif = function(user, product){
    ...
    .then(() => {
      return twilio.send({
        ...
        body: `Hi ${user.firstname}!  We wanted to let you know that ${product.name} is back in stock.  We have added it to your cart at https://demo.mongodb.com/cart` 
      });
    })
    .then(() => {
  ...
  try{
    return users.find({"notify.id": product.id}).toArray().then(notifyUsers => {
